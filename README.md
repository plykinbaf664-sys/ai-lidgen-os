...
## Исправление целостности исходящих писем (11 августа 2026)

Устранён критический дефект, из-за которого API заменял полный `body` письма превью длиной 240 символов. Сокращённое значение могло сохраняться обратно в очередь и отправляться через SMTP.

Выполнено:

* API списка initial outreach и follow-up теперь возвращает полный текст письма;
* добавлена единая проверка целостности текста перед сохранением, постановкой в очередь и SMTP-отправкой;
* письмо с признаком старого обрезанного preview блокируется до повторной проверки;
* 14 неотправленных писем восстановлены из исходных данных контактов;
* восстановленные письма переведены в `needs_review`, прежнее одобрение аннулировано;
* исторически отправленные записи не переписываются;
* кнопка «Открыть письмо» доступна во всех состояниях карточки;
* до отправки письмо можно открыть и отредактировать;
* `queued`, `sending` и `sent` открываются в режиме просмотра;
* после открытия UI загружает полную запись и не использует сокращённый текст как редактируемое значение.

Защита отправки находится в:

```text
lib/leadgen/outreach-body-integrity.ts
lib/leadgen/outreach-storage.ts
lib/leadgen/outreach-processor.ts
lib/leadgen/local-outreach-processor.ts
```
...
npm run lint — OK
npm run build — OK
scripts/check-project.sh — OK
```

На момент исправления обрезанных неотправленных писем в локальном хранилище не осталось.
